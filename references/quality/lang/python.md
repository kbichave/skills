> **Language ref for the `code-reviewer` agent.** Thresholds: `lint/python/adapter.json`. Plugin gates: `ruff`, `mypy --strict` (all public fns typed, explicit returns, no bare `Any`), `bandit`. Docstrings: Google style with Args/Returns/Raises on public APIs (`DOC-*`).
> Vendored from [awesome-skills/code-review-skill](https://github.com/awesome-skills/code-review-skill) (MIT). See NOTICE.
