> **Rule mapping:** `CONC-001` (shared mutable state), `CONC-002` (timeout + bounded retry), `CONC-003` (idempotency), `CONC-004` (check-then-act races). Consult when reviewing threaded/async/goroutine code.
