> **Rule mapping:** `ERR-001` (no swallowed exceptions), `ERR-002` (fail closed), `ERR-003` (actionable messages), `ERR-004` (validate at boundaries). Consult when reviewing try/except and error paths.
