...
3. **Prime status.** Call `tracker.prime()`, then read `{planning_dir}/.deepstate/prime.md`.

4. **Find next step.** Call `tracker.ready()`.
...
If state.json is corrupted (parse error): tell the user, do not auto-recreate.
